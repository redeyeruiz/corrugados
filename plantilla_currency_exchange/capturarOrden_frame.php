<?php
    ...
    cancelar();
    $_SESSION['idRepresentante']="IDrep1";
    clientValues();
    calcularPrecio();
    update_dir();
    $_SESSION['dirCompleta']="prueba dir";
   ...
   <!-- section 
    ...
    <form class="formCO" method="POST" action="capturarOrden_frame.php">
        ...
        <div class="row g-3">
            ...
            <input class="inputCO" type="number" name='ordenCompra' placeholder="Ingrese el número de órden de compra" required value="<?php echo htmlspecialchars($_POST['ordenCompra'] ?? '', ENT_QUOTES); ?>"></input>
            </p>
    ...
    function agregar_tabla(){
        echo $_SESSION['direntC'];
        echo $_SESSION['idCompania'];
        ...
        if( isset($_POST['agregarArt']) && isset($_SESSION['idCompania']) && isset($_SESSION['direntC'])){
            ...
            if(!isset($_SESSION['folio']))
            {
                ...
                $query="SELECT MAX(ordenCompra) AS folio FROM Orden WHERE 1";
            ...
            $idOrden            =$_SESSION['IDorden'] =$_POST['IDorden'];
            ...
            $idArticulo                                 ="idtest";
            $ordenCompra=$_SESSION['ordenCompra']       = $_POST['ordenCompra'];
        ...
        if(isset($_POST["guardar"])){
            ...
            if(isset($_SESSION['idCliente']) && !isset($_SESSION['numOrdenes'])){
                $idCliente=$_SESSION['idCliente'];
                $query="SELECT MAX(numOrdenes) AS numOrdenes FROM Orden WHERE idCliente = '$idCliente'";
                $sql=mysqli_query($conn,$query);
                $reg=mysqli_fetch_object($sql);
                if ($reg==mysqli_fetch_array($sql)){
                    echo "error no existe tal cliente ('numOrdenes')";
                }else{
                    $_SESSION['numOrdenes']=strval( intval($reg->numOrdenes) +1);
                }
            }
            if(isset($_SESSION['idCliente'])){
                $query="SELECT MAX(ordenCompra) AS ordenCompra FROM Orden WHERE 1 ";
                $sql=mysqli_query($conn,$query);
                $reg=mysqli_fetch_object($sql);
                if ($reg==mysqli_fetch_array($sql)){
                    echo "error no existe tal orden de compra ('orden compra')";
                }else{
                    $_SESSION['ordenCompra']=strval( intval($reg->numOrdenes) +1);
                }
            }  
            
            if(isset($_SESSION['queries'])){
                $numOrdenes=$_SESSION['numOrdenes'];
                $dirEnt=$_SESSION['direntC'];
                $idOrden=$_SESSION['IDorden'];
                $idCompania=$_SESSION['idCompania'];
                $idCliente=$_SESSION['inputID'];
                $idOrden=$_SESSION['IDorden'];
                ...
                $query="INSERT INTO orden VALUES('$idOrden','$idCompania','$idCliente','$dirEnt','0','$ordenCompra','$fechaOrden','null','null','null','null','null','null','null','$total','0','0','0','0','0','0','0','$numOrdenes')";
    ...
    function clientValues(){
        ...
        if(isset($_SESSION['nombreClienteDT'])){
            ...
            $conn=conecta_servidor();
            $query="SELECT idCliente,estatus,idLista,saldoOrden,bloqueo,idCompania FROM cliente WHERE nombreCliente = '$nombreCliente'";
            $sql=mysqli_query($conn,$query);
            $reg=mysqli_fetch_object($sql);
            if ($reg==mysqli_fetch_array($sql)){
    ...
    function calcularPrecio(){
        if(isset($_POST['calcularP']) && isset($_POST['cantidad'])){
